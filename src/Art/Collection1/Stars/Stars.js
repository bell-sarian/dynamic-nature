...
export default function Stars() {
    ...
    return (React.createElement("div", { className: "galaxy" }, domContent.map((item, index) => {
        var xposition = Math.floor(Math.random() * window.innerWidth);
        var yposition = Math.floor(Math.random() * window.innerHeight);
        var star_type = Math.floor((Math.random() * 3) + 1);
        return (React.createElement("div", { className: "star star-type" + item.type, style: { top: item.yAxis, left: item.xAxis }, key: index }));
    })));
}
;
